...
export function ArticlePreviewModal({
  ...
  useEffect(() => {
    ...
    fetch(
      `/api/articles/${encodeURIComponent(active.article.slug)}?locale=${locale}`,
  ...
  const headline = splitDisplayTitle(active.article.title).headline;
  const imageUrl = active.origin.imageUrl;
  const category =
    categoryLabels[active.article.category as keyof typeof categoryLabels] ??
    active.article.category;
  const hasFlight = Boolean(imageUrl) && phase !== "ready";
  const contentReady = phase === "ready";
  const flyStyle =
    phase === "from" || !target
      ? originStyle(active.origin)
      : target;
      ...
      >
        ...
        <div className="grid min-h-0 flex-1 grid-cols-[minmax(16rem,0.85fr)_minmax(0,1.25fr)] gap-7 px-5 pb-5 pt-1 max-nav:grid-cols-1 max-nav:gap-4 max-nav:overflow-y-auto max-nav:px-4 max-nav:pb-4">
          <div className="min-h-0 max-nav:mx-auto max-nav:w-full max-nav:max-w-[22rem]">
            ...
            >
              {(detail?.coverUrl ?? imageUrl) && (
                <Image
                  src={detail?.coverUrl ?? imageUrl!}
                  alt={detail?.imageAlt || active.article.imageAlt || headline}
                  fill
                  sizes="(max-width: 900px) 90vw, 42vw"
                  className={cn(
                    "object-cover transition-opacity duration-200",
                    hasFlight ? "opacity-0" : "opacity-100",
                  )}
                  priority
                />
              )}
